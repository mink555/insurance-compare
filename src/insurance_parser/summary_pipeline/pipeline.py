...
parse → normalize → classify → export → (LLM enrich)

Graceful degradation:
  - summary_pdf만 있으면 summary 기준으로 파싱
  - terms_pdf만 있으면 경고 후 빈 결과 (TermsParser 미구현)
  - require_complete=False(기본값): 단일 문서 허용
  - require_complete=True: BOTH 아니면 ValueError

사용 예시:
    from insurance_parser.summary_pipeline import run_pipeline, DocumentBundle

    result = run_pipeline(DocumentBundle(
        company_name="라이나생명",
...
def _warn_if_high_other_ratio(
    ...
    if ratio > _OTHER_RATIO_THRESHOLD:
        ...
        logger.warning(
            "[Stage3/classify] other 비율 %.1f%% > %.0f%% — "
            "benefit_category_keywords.json 키워드 추가 필요. 미분류 (%d건): %s",
            ratio * 100, _OTHER_RATIO_THRESHOLD * 100, len(names), names,
        )
...
def _parse_summary_pdf(pdf_path: str, company_name: str) -> tuple[Optional[dict], list[str]]:
    ...
    try:
        data = parser_cls().parse_pdf(Path(pdf_path))
...
def run_pipeline(
    ...
    *,
    ...
    실패 시에도 errors 채운 결과를 반환한다 (앱이 죽지 않음).
    """
    if require_complete:
    ...
    # terms_pdf만 있는 경우: TermsParser 미구현 → 경고 후 종료
    if not has_summary and has_terms:
        result.warnings.append(
            "TERMS_ONLY: summary_pdf 없음 — 급부 정보 제한적 추출 상태. "
            "상품요약서를 추가하면 더 정확한 결과를 얻을 수 있습니다."
        )
        result.warnings.append("⚠️ terms_pdf 파싱은 현재 미구현 — 급부 정보 없음")
        return result

    summary_data: Optional[dict] = None
    insurance_type = detect_insurance_type(
        bundle.product_name,
        bundle.summary_pdf or bundle.terms_pdf or "",
    )
    ...
    if hint == DocType.UNKNOWN:
        resolved_type, summary_data, parse_warnings = try_parse_unknown(
            bundle.summary_pdf, bundle.company_name
        )
        result.warnings.extend(parse_warnings)
        result.warnings.append(f"문서 타입 불명확 → fallback 파싱: {resolved_type.value}")
    elif hint == DocType.TERMS:
        result.warnings.append(
            "요약서 경로에 약관이 입력됨 — terms_pdf 파싱은 미구현, 빈 결과 반환"
        )
    else:
    ...
    raw_benefit_count = sum(len(c.get("benefits", [])) for c in raw_contracts if isinstance(c, dict))
    logger.info(
    ...
    canonical: list[CanonicalBenefit] = normalize_summary_data(
        summary_data, company_name=bundle.company_name, product_name=bundle.product_name,
    )
    for b in canonical:
    ...
    logger.info(
        "[Stage2/normalize] %s/%s: CanonicalBenefit %d건 | sample: %s",
    ...
    try:
        ...
        logger.info(
            ...
            bundle.company_name, bundle.product_name, enriched_count, len(rows),
        )
    except Exception as e:
